...
The same operator (Grok Bot) helped design the harness framing, produced the agent decisions under test, and the outcomes were scored with deterministic `assert_policy` assertions. Deterministic assertions are an independent *scoring* layer, but this is **not** an independent multi-model evaluation and must not be cited as one. Prefer a held-out task set or a separate execution path for any stronger claim.
